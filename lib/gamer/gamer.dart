...
// Konstanta yang mendefinisikan tinggi dan lebar dari game pad
...
enum GameStates {
  ...
  /// Permainan dalam mode jeda, penurunan blok akan dihentikan
  paused,

  /// Permainan sedang berlangsung, blok sedang turun
  /// Tombol dapat diinteraksi
  running,

  /// Sedang mereset permainan
  /// Setelah mereset, status [GameController] akan beralih ke [none]
  reset,

  /// Blok jatuh telah mencapai dasar, saat ini sedang mematikan blok dalam matriks permainan
  /// Setelah penyelesaian pengaturan, akan segera memulai tugas penurunan blok berikutnya
  mixing,

  /// Sedang menghapus baris
  /// Setelah penghapusan selesai, akan segera memulai tugas penurunan blok berikutnya
  clear,

  /// Blok turun dengan cepat ke dasar
  drop,
}
...
class GameControl extends State<Game> with RouteAware {
  Duration _totalPlaytime = Duration();
  GameControl() {
...
/// Pada metode [build], campurkan [_data] untuk membentuk matriks baru
/// Matriks [_mask] memiliki lebar dan tinggi yang sama dengan [_data]
/// Untuk setiap _mask[x, y]:
///   - Jika nilai adalah 0, maka tidak ada pengaruh pada [_data]
///   - Jika nilai adalah -1, itu berarti baris tersebut tidak ditampilkan dalam [_data]
///   - Jika nilai adalah 1, itu berarti baris tersebut di-highlight dalam [_data]
final List<List<int>> _mask = [];
  ...
  Future<void> _mixCurrentIntoData({VoidCallback? mixSound}) async {
    ...
    if (clearLines.isNotEmpty) {
      ...
      //Menghapus semua baris yang dihilangkan
      clearLines.forEach((line) {
    ...
    //_current telah digabungkan ke dalam _data, jadi tidak lagi diperlukan.
    _current = null;

    //Memeriksa apakah permainan telah berakhir, yaitu dengan memeriksa apakah ada elemen dengan nilai 1 pada baris pertama.
    if (_data[0].contains(1)) {
      reset();
      return;
    } else {
      //Permainan belum berakhir, memulai jatuhnya blok untuk putaran berikutnya.
      _startGame();
    }
  }
...
void _startGame() {
  ...
  // Start a timer to track total playtime
  Timer.periodic(Duration(minutes: 3), (timer) {
    _totalPlaytime += Duration(minutes: 3);
    
    // Increase the level every 3 minutes
    _level = (_totalPlaytime.inMinutes ~/ 3 + _LEVEL_MIN).clamp(_LEVEL_MIN, _LEVEL_MAX);
